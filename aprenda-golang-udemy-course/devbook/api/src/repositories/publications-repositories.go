...
func (repository PublicationRepository) UnlikePublication(publicationId uint64) error {
	statement, erro := repository.db.Prepare(`update publications set likes =  CASE WHEN likes > 0 THEN likes - 1 ELSE 0 END where id = ?`)
